...
$frm = new TForm('Cadastro de subclasse financeira', 530, 1000);



Subclasse_financeiraDAO::executeSql("set names utf8"); // configurando acentuação no mysql
...
$frm->addSelectField('classe','Classe:',false,'SELECT codigo,descricao FROM classe_financeira',20,true,20,null,true,null,null,true)->setCss('font-size','14px');
...
function callbackModaBox(data, doc )
{
    ...
    {
        if (fwConfirm('Deseja excluir a subclasse financeira ?',
                function(r) {
                    if (r == true)
                    {
                             fwAjaxRequest({
