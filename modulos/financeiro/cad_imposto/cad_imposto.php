...
$frm = new TForm('Cadastro de Imposto', 530, 1000);
...
$frm->addTextField('codigo', 'Código:', 10, false, 10, null, null, null, null, true)->addEvent('onblur', 'buscaImposto(this)')->addEvent('onFocus', 'novo()')->setCss('font-size','14px');

$frm->setOnlineSearch('codigo', 'imposto'
        , 'nome|Pesquisa por nome:||||||true|true'
        , false
        , true
        , true // se for encontrada apenas 1 opÃ§Ã£o fazer a seleï¿½ï¿½o automaticamente
        , 'codigo|Código,nome|Nome,situacao|Situação'
...
$frm->addTextField('nome', 'Imposto:', 50, true, 30, null, true, null, null, true)->setCss('font-size', '14px')->setCss('text-transform', 'uppercase')->addEvent('onblur', 'upperCase(this)');
...
<script>
    ...
    function buscaImposto(campoChave, valorChave)
    {
    ...
    {
        if (fwConfirm('Deseja excluir o imposto selecionado ?',
                function (r) {
                    if (r == true)
                    {
                        fwAjaxRequest({
