...
$frm->addHiddenField('grupo', $strLabel,10);
...
if (empty($_POST['psq_cid']))
{
    ...
    $dados = Grupo_financeiroDAO::selectEspecifico(null, 'select * from grupo_financeiro where descricao= '.$_POST['psq_cid']);  
    $g = new TGrid('gd', 'Grupo financeiro cadastrado', $dados, null, null, 'codigo', null, 15, null);
    $g->addColumn('codigo', 'Código', 400, 'left');
    $g->addColumn('descricao', 'Descrição', 300, 'left');
    $g->addButton('Alterar', null, 'btnAlterar', 'grideAlterar()', null, 'editar.gif', null, 'Alterar grupo financeiro');
    $g->addButton('Cancelar', null, 'btnCancelar', 'grideCancelar()', null, 'lixeira.gif', null, 'Excluir grupo financeiro');
...
<script>
    ...
    {
        fwGetGrid('financeiro/cad_subgrupo_financeiro/cad_subgrupo_financeiro.php', 'html_gride', {"action": "criar_gride", "psq_cid": ""});
    ...
    {
        if (fwConfirm('Deseja cancelar o subgrupo financeiro?',
                function(r) {
                    if (r == true)
                    {
                        fwAjaxRequest({
