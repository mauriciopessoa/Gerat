...
if (empty($_POST['psq_cid']))
{
    ...
    $dados = Classe_laudoDAO::selectEspecifico(null, 'select * from classe_laudo where descricao= '.$_POST['psq_cid']);  
    $g = new TGrid('gd', 'Classe de laudo cadastrado', $dados, null, null, 'codigo', null, 15, null);
    $g->addColumn('codigo', 'Código', 400, 'left');
    $g->addColumn('descricao', 'Descrição', 300, 'left');
    $g->addButton('Alterar', null, 'btnAlterar', 'grideAlterar()', null, 'editar.gif', null, 'Alterar classe laudo');
    $g->addButton('Cancelar', null, 'btnCancelar', 'grideCancelar()', null, 'lixeira.gif', null, 'Excluir classe laudo');
...
<script>
    ...
    {
        fwGetGrid('atendimento/cad_classe_laudo/cad_classe_laudo.php', 'html_gride', {"action": "criar_gride", "psq_cid": ""});
