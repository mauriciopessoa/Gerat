...
//---------------------------------------------------------------------------------------------
function app_confirm(message,confirmCallBack,cancelCallBack)
{
	parent.app_blockScreen('#000000',80);
	jQuery.get(pastaBase+'includes/confirm_dialog.html', function( data )
  	{
    	jQuery("#tdBlockScreen").append(utf8_decode(data));
  		jQuery("#tableDialogBox").corner("round");
    	jQuery("#tdDialogBoxText").html(message);
  		jQuery("#tableDialogBox").show("fast");
    	jQuery("#DialogBoxButtonYes").bind('click', function(){
    		app_UnBlockScreen();
    		if( confirmCallBack )
    		{
				var app_iframe = document.getElementById('app_iframe'); // não utilizar fwGetObj() aqui
	            app_iframe.contentWindow.fwExecutarFuncao(confirmCallBack);
			}
    		} );
    	jQuery("#DialogBoxButtonNo").bind('click', function(){
    		app_UnBlockScreen();
    		if( cancelCallBack )
    		{
				var app_iframe = document.getElementById('app_iframe'); // não utilizar fwGetObj() aqui
	            app_iframe.contentWindow.fwExecutarFuncao(cancelCallBack);
			}
    		} );
		}
	);
...
/*
 * Função para substituir a funcao alert() do javascript
 * Gera uma caixa de mensagem que pode ser arrastada
 * Exemplo: app_alert('Minha Mensagem',{'title':'Titulo da Janela','callback':function(result){} } );
 *			app_alert('Mensagem Teste ');
 */
function app_alert(message,jsonParams,theme)
{
	...
	jsonParams.theme	= theme||jsonParams.theme;
	...
	jQuery.alerts.dialogClass = jsonParams.theme;
...
function app_modalGeneric(paramModal)
	...
	paramModal.img = (paramModal.img==null) ? '../css/imagens/alert/help.gif':paramModal.img;
